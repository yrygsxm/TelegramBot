...
const TELEGRAM_MINI_APP_PUBLIC_KEY_PRODUCTION =
  'e7bf03a2fa4602af4580703d88dda5bb59f32ed8b02a56c187fe7d34caed242d'
const VERIFICATION_HANDOFF_MESSAGE_PREFIX = '✅ 验证已完成'
...
function createBotApp(env) {
  ...
  const webhookSecret = String(env.ENV_TELEGRAM_WEBHOOK_SECRET || '').trim()
  const internalSecret = String(env.ENV_INTERNAL_SECRET || '').trim()
  ...
  function ensureWebhookSecretConfigured() {
    ensureCoreConfigured()
    if (!webhookSecret) {
      throw new Error('Missing required configuration: ENV_TELEGRAM_WEBHOOK_SECRET')
    }
  }

  function ensureInternalSecretConfigured() {
    ensureCoreConfigured()
    if (!internalSecret) {
      throw new Error('Missing required configuration: ENV_INTERNAL_SECRET')
    }
  }
  ...
  async function kvGetJson(key) {
    ...
    return storage.get(key, {
      type: 'json',
      cacheTtl: DEFAULTS.kvReadCacheTtlSeconds,
    })
  }
  ...
  function answerWebAppQuery(msg = {}) {
    return requestTelegram('answerWebAppQuery', makeReqBody(msg))
  }
  ...
  function timingSafeEqualStrings(userInput, expectedValue) {
    if (!expectedValue) return false
    const userValue = encodeText(userInput)
    const secretValue = encodeText(expectedValue)
  ...
  async function signHmacSha256(keyBytes, message) {
    ...
    return new Uint8Array(await crypto.subtle.sign('HMAC', key, encodeText(message)))
  }
  ...
  function toBase64UrlFromText(text) {
    return toBase64UrlFromBytes(encodeText(text))
  }
  ...
  async function getVerificationGrantKey() {
    const grantSecret = String(internalSecret || webhookSecret || token || '').trim()
    if (!grantSecret) {
      throw new Error('Missing secret for verification grants')
    }
    return crypto.subtle.importKey(
      'raw',
      encodeText(grantSecret),
      { name: 'HMAC', hash: 'SHA-256' },
      false,
      ['sign', 'verify']
    )
  ...
  async function signVerificationGrant(payload) {
    ...
    const key = await getVerificationGrantKey()
    const signature = await crypto.subtle.sign('HMAC', key, encodeText(serialized))
  ...
  async function validateTelegramInitData(initData, expectedChatId) {
    const rawInitData = String(initData || '').trim()
    if (!rawInitData) {
      return { ok: false, message: '请从 Telegram 对话中的验证按钮重新打开此页面。' }
    }
    ...
    if (!hash || !rawUser || !Number.isFinite(authDate)) {
      return { ok: false, message: 'Telegram 会话数据不完整，请重新打开验证页面。' }
    }

    const webAppDataKey = await signHmacSha256(encodeText('WebAppData'), token)
    const buildDataCheckString = excludeSignature => {
      const entries = []
    ...
    const computedHash = bytesToHex(
      await signHmacSha256(webAppDataKey, buildDataCheckString(false))
    )
    const legacyComputedHash = params.has('signature')
      ? bytesToHex(await signHmacSha256(webAppDataKey, buildDataCheckString(true)))
      : ''
    const botId = String(token || '').split(':')[0]
    const signatureValue = String(params.get('signature') || '').trim()
    let thirdPartySignatureValid = false

    if (signatureValue && /^\d+$/.test(botId)) {
      try {
        const verificationText = `${botId}:WebAppData\n${buildDataCheckString(true)}`
        const ed25519Key = await crypto.subtle.importKey(
          'raw',
          hexToBytes(TELEGRAM_MINI_APP_PUBLIC_KEY_PRODUCTION),
          { name: 'Ed25519' },
          false,
          ['verify']
        )
        thirdPartySignatureValid = await crypto.subtle.verify(
          { name: 'Ed25519' },
          ed25519Key,
          base64UrlToBytes(signatureValue),
          encodeText(verificationText)
        )
      } catch (error) {
        console.error('telegram initData signature verification failed', error)
      }
    }

    if (
      !timingSafeEqualStrings(computedHash, hash) &&
      !timingSafeEqualStrings(legacyComputedHash, hash) &&
      !thirdPartySignatureValid
    ) {
      return { ok: false, message: 'Telegram 会话签名校验失败，请重新打开验证页面。' }
    }

    const authDateMs = authDate * 1000
    if (
      !Number.isFinite(authDateMs) ||
      authDateMs > Date.now() + 5 * 60 * 1000 ||
      Date.now() - authDateMs > DEFAULTS.verificationSessionTtlMs
    ) {
      return { ok: false, message: 'Telegram 会话已过期，请重新打开验证页面。' }
    }
    ...
    try {
      ...
      return { ok: false, message: 'Telegram 用户信息解析失败，请重新打开验证页面。' }
    }

    if (!user?.id || String(user.id) !== String(expectedChatId)) {
      return { ok: false, message: '当前验证会话与用户不匹配，请重新获取验证按钮。' }
    }
  ...
  function normalizeBaseUrl(requestOrigin = '') {
    const trimmedPublicBaseUrl = String(publicBaseUrl || '').trim().replace(/\/+$/, '')
    if (trimmedPublicBaseUrl) {
      if (/^https?:\/\//i.test(trimmedPublicBaseUrl)) {
        return trimmedPublicBaseUrl
      }
      return `https://${trimmedPublicBaseUrl.replace(/^\/+/, '')}`
    }
  ...
  function getAdminHelpText() {
    return [
      ...
      '4. 排查验证状态：/debugverify（引用用户消息）或 /debugverify 用户UID',
      '5. 关键词黑名单命令：',
...
<html lang="zh-CN">
    ...
    <main>
      ...
      <div id="panel" class="panel">
        ${
          canRenderWidget
            ? `<div id="challengeArea">
              ...
              ></div>
                <div class="hint">由于网络原因可能验证时间较长，请耐心等待验证结果。验证完成后即可直接关闭页面并返回对话。</div>
              </div>
              <div id="successArea" class="hidden">
                <div class="success-title">验证已经完成</div>
                <div class="success-subtitle">现在可以直接关闭页面并返回对话。</div>
              </div>`
        ...
        <div class="actions">
          ${
            canRenderWidget
              ? '<button id="retry" class="secondary" type="button" onclick="retryTurnstile()">重新加载验证</button><button id="closeBtn" class="hidden" type="button" onclick="closePage()">关闭页面</button>'
    ...
    <script>
      ...
      const tgWebApp =
        ...
        window.Telegram.WebApp
          ...
          : null;
      const initData =
        tgWebApp && typeof tgWebApp.initData === 'string' ? tgWebApp.initData : '';
      ...
      function showSuccessState(message) {
        ...
        setStatus(message || '验证已经完成，现在可以关闭页面并返回对话。', 'success');
      }
      ...
      async function submitToken(token) {
        ...
        if (!initData) {
          setStatus('请务必从 Telegram 对话底部的验证按钮进入此页面。', 'error');
        ...
        try {
          ...
          showSuccessState(result.message || '验证已经完成，现在可以关闭页面并返回 Telegram。');
        } catch (error) {
      ...
      if (statusEl && statusEl.dataset.state === 'success') {
        showSuccessState('当前会话已经验证成功，请返回对话。');
        ...
        setStatus('请从 Telegram 对话底部的 👋 验证按钮进入此页面。', 'error');
      }
    </script>
  </body>
</html>`
  ...
  async function sendCaptcha(chatId, forceNew = false, requestOrigin = '') {
    if (hasTurnstileConfig) {
      ...
      return sendMessage({
        chat_id: chatId,
        text: '为了继续私聊，请先完成 Cloudflare 人机验证。',
  ...
  async function handleAdminMessage(message) {
    ...
    if (/^\/debugverify(?:@[\w_]+)?(?:\s+\d+)?$/.test(text)) {
      return handleDebugVerify(message)
    }
    if (/^\/block$/.test(text)) {
  ...
  async function onMessage(message, requestOrigin = '') {
    ...
    if (rawText.startsWith(VERIFICATION_HANDOFF_MESSAGE_PREFIX)) {
      await handleVerificationGrantMessage(message)
      return
    }
    ...
    if (message.web_app_data) {
      await sendMessage({
        chat_id: message.chat.id,
        text: '验证方式已更新，请重新点击消息底部的 👋 验证按钮完成一次新的验证。',
  ...
  async function handleVerificationGrantMessage(message) {
    const chatId = String(message.chat?.id || '')
    const rawText = String(message.text || '')
    const grantToken = rawText
      .split('\n')
      .map(part => part.trim())
      .filter(Boolean)
      .slice(1)
    ...
    const grant = await verifyVerificationGrant(grantToken, chatId)
    if (!grant || grant.kind !== 'turnstile_verified') {
      ...
      await sendMessage({
        chat_id: chatId,
        text: '验证回执无效，请重新点击底部的 👋 验证按钮完成一次新的验证。',
      })
      return
    }
    ...
    await consumeVerificationSession(existingSession.sessionId, {
      completedAt: existingSession.completedAt || new Date().toISOString(),
      handoffReceivedAt: new Date().toISOString(),
      hostname: grant.hostname || existingSession.hostname || '',
      remoteIp: grant.remoteIp || existingSession.remoteIp || '',
      verifiedVia: 'turnstile-inline-webapp-handoff',
      verifiedByUserId: grant.verifiedByUserId || existingSession.verifiedByUserId || '',
      verifiedQueryId: grant.verifiedQueryId || existingSession.verifiedQueryId || '',
      verifiedAuthDate: grant.verifiedAuthDate || existingSession.verifiedAuthDate || '',
    })
  ...
  async function handleResetVerify(message) {
    ...
    const whitelistState = await getWhitelistState(String(guestChatId))
    if (whitelistState.whitelisted) {
      await clearWhitelistState(String(guestChatId))
    }
    ...
    await sendMessage({
      ...
      text:
        '管理员已重置你的验证状态。请重新发送 /start 或直接发送消息，先完成欢迎菜单选择，再完成 Cloudflare 验证。',
    })

    return sendMessage({
      chat_id: adminUid,
      text: whitelistState.whitelisted
        ? `已重置用户 ${guestChatId} 的验证状态，并已自动移出白名单。\n\n接下来的正常流程是：先重新显示欢迎菜单，用户点击任一欢迎按钮后，才会弹出 Cloudflare 验证。`
        : `已重置用户 ${guestChatId} 的验证状态。\n\n接下来的正常流程是：先重新显示欢迎菜单，用户点击任一欢迎按钮后，才会弹出 Cloudflare 验证。`,
    })
  }

  async function handleDebugVerify(message) {
    const text = (message.text || '').trim()
    const uidMatch = /^\/debugverify(?:@[\w_]+)?\s+(\d+)$/.exec(text)
    const guestChatId = uidMatch?.[1] || (await getGuestChatIdFromReply(message))

    if (!guestChatId) {
      return sendMessage({
        chat_id: adminUid,
        text: '请使用：/debugverify 用户UID\n或引用一条机器人转发出的用户消息后发送 /debugverify',
      })
    }

    const chatId = String(guestChatId)
    const [
      verified,
      welcomeShown,
      welcomeSelection,
      ...
      kvGetJson(`verified-${chatId}`),
      kvGetJson(`welcome-shown-${chatId}`),
      kvGetJson(`welcome-selection-${chatId}`),
      getWhitelistState(chatId),
      getBlockedState(chatId),
      kvGetJson(getActiveVerificationSessionKey(chatId)),
    ])
    ...
    return sendMessage({
      ...
      text: [
        `验证排查：UID ${chatId}`,
        '',
        `CAPTCHA_ENABLED：${captchaEnabled ? 'true' : 'false'}`,
        `Turnstile 配置：${hasTurnstileConfig ? '已启用' : '未启用'}`,
        `Site Key：${turnstileSiteKey ? '已配置' : '缺失'}`,
        `Secret Key：${turnstileSecretKey ? '已配置' : '缺失'}`,
        `PUBLIC_BASE_URL：${publicBaseUrl || '未设置（将使用请求域名）'}`,
        `verified：${verified ? 'true' : 'false'}`,
        `welcome-shown：${welcomeShown ? 'true' : 'false'}`,
        `welcome-selection：${welcomeSelection?.completed ? `已完成 (${welcomeSelection.action || 'unknown'})` : '未完成'}`,
        `whitelisted：${whitelistState.whitelisted ? 'true' : 'false'}`,
        `blocked：${blockedState.blocked ? 'true' : 'false'}`,
        `active-session：${activeSession?.sessionId ? '存在' : '无'}`,
      ].join('\n'),
    })
  }
  ...
  function isInternalRequestAuthorized(request) {
    ...
    return (
      timingSafeEqualStrings(auth, `Bearer ${internalSecret}`) ||
      timingSafeEqualStrings(secretHeader, internalSecret)
    )
  }
  ...
  async function handleWebhook(request, ctx) {
    ...
    if (!timingSafeEqualStrings(receivedSecret, webhookSecret)) {
  ...
  async function registerWebhook(requestUrl) {
    ...
    const response = await fetch(
      apiUrl('setWebhook', { url: webhookUrl, secret_token: webhookSecret })
    )
    const result = await response.json()
  ...
  async function handleInternalWeeklyReport(request) {
    ensureInternalSecretConfigured()
  ...
  async function handleVerificationSubmit(request) {
    ...
    const initDataValidation = await validateTelegramInitData(initData, session.chatId)
    if (!initDataValidation.ok) {
      return jsonResponse({ ok: false, message: initDataValidation.message }, 403)
    }
    if (!initDataValidation.queryId) {
      return jsonResponse(
        { ok: false, message: '当前 Telegram 会话不支持验证回执，请关闭页面后重新从验证按钮进入。' },
        409
      )
    }
    ...
    const grantToken = await signVerificationGrant({
      ...
      remoteIp,
      verifiedByUserId: String(initDataValidation.user.id),
      verifiedQueryId: initDataValidation.queryId,
      verifiedAuthDate: initDataValidation.authDate,
    })
    const handoffResponse = await answerWebAppQuery({
      web_app_query_id: initDataValidation.queryId,
      result: {
        type: 'article',
        id: crypto.randomUUID(),
        title: '验证完成',
        input_message_content: {
          message_text: `${VERIFICATION_HANDOFF_MESSAGE_PREFIX}\n<tg-spoiler>${escapeHtml(
            grantToken
          )}</tg-spoiler>`,
          parse_mode: 'HTML',
    ...
    if (!handoffResponse?.ok) {
      console.error('answerWebAppQuery failed', handoffResponse)
      return jsonResponse(
        {
          ok: false,
          message: '验证已通过，但回传对话失败。请关闭页面后重新从验证按钮进入再试一次。',
        },
        502
      )
    }
